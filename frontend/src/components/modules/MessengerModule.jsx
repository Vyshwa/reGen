                          ...
                          {message.attachments.map((att, idx) => (
                            <div key={`${message.id}-att-${idx}`} className="rounded-md border border-border p-2 bg-background/40">
                              {att.type?.startsWith('image/') && (
                                <img src={att.data} alt={att.name} className="max-h-40 rounded-md object-contain mb-2" />
                              )}
                              <a href={att.data} download={att.name} className="text-xs underline flex items-center gap-2" title={att.name}>
                                <Paperclip className="h-3 w-3" />
                                {truncateName(att.name)} ({Math.round((att.size || 0) / 1024)} KB)
                              </a>
                            </div>
                          ))}
                        </div>

...
// Maintenance Mode Middleware for all other API routes
app.use((req, res, next) => {
  // Bypasses for essential endpoints
  const bypass = [
    '/api/auth/login',
    '/api/health',
...
app.post('/api/system/factory-reset', async (req, res) => {
  ...
  try {
    ...
    for (const key in collections) {
      if (key === 'users') {
        // Keep only the Super Admin 'Vyshwa'
        await User.deleteMany({ username: { $ne: 'Vyshwa' } });

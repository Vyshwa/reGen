...
export default function DashboardHome({ userProfile, onNavigate }) {
  ...
  const totalStaff = users.filter(u => u.role.hasOwnProperty('staff')).length;
  const totalDepartments = new Set(users.map(u => u.department).filter(Boolean)).size;
  
  const todayDate = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
  const activeUsersToday = allAttendance.filter(a => a.date === todayDate).length;
  ...
  const myDepartmentStaff = userProfile.department 
    ? users.filter(u => u.role.hasOwnProperty('staff') && u.department === userProfile.department)
    : users.filter(u => u.role.hasOwnProperty('staff'));
    
  const activeMyStaffToday = allAttendance.filter(a => {
    const user = users.find(u => u.id.toText() === a.userId.toText());
    return a.date === todayDate && user && (userProfile.department ? user.department === userProfile.department : user.role.hasOwnProperty('staff'));
  }).length;
    ...
    return (
      <div className="space-y-6 pb-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold">Hello, {userProfile.name.split(' ')[0]}</h2>
          ...
          <div className="h-10 w-10 rounded-full bg-muted overflow-hidden border-2 border-background shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
            {stats.filter(stat => stat.show).slice(0, 4).map((stat) => (

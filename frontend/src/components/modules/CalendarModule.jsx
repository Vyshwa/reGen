...
export default function CalendarModule({ userProfile }) {
  ...
  const visibleHolidays = useMemo(() => {
    ...
    return holidays.filter(h => String(h.name || '').toLowerCase() !== 'sunday');
  }, [holidays, isAdmin]);
  ...
  const birthdayDays = useMemo(() => {
    ...
    const isStaffUser = (u) => {
      ...
      return Object.prototype.hasOwnProperty.call(u.role, 'staff');
    };
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-7 gap-2">
            {monthMatrix.flat().map((cell, idx) => (
              <div
                key={idx}
                className={`rounded-md p-2 h-16 flex flex-col justify-between ${cell.inMonth ? '' : 'opacity-40'} ${cell.isHoliday ? 'bg-muted text-muted-foreground' : 'bg-background border'}`}
                style={
                  cell.leaveName
              ...
              >
                ...
                {cell.isHoliday && (
                  <div className="text-[10px]">Holiday</div>
                )}
                {cell.leaveName && (
                  <div className="text-[10px] font-semibold">Leave: {String(cell.leaveName)}</div>
                )}
                {cell.birthdayLabel && (
                  <div className="text-[10px] font-semibold">Happy Birthday '{String(cell.birthdayLabel)}'</div>
                )}
              </div>
            ))}
          </div>

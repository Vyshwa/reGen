  ...
  return {
    ...
    saveCallerUserProfile: async (profile) => {
      ...
      const user = {
        ...
        gender: profile.gender,
        aadhaar: profile.aadhaar,
        role: Object.keys(profile.role)[0] || 'staff', // simplistic role mapping
        email: profile.email,
    ...
    createTask: async (task) => {
        ...
        const payload = {
            ...
            startDate: new Date(Number(task.startDate) / 1000000).toISOString(),
            endDate: new Date(Number(task.endDate) / 1000000).toISOString(),
            dueDate: new Date(Number(task.endDate) / 1000000).toISOString()
        };
    ...
    recordAttendance: async (attendance) => {
        const payload = {
            ...
            checkIn: new Date(Number(attendance.checkInTime) / 1000000).toISOString(),
